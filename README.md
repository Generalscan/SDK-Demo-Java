...
* Copy the SDK aar file (let say generalscan-sdk-1.0.aar) to app/libs
* Add the flatDir setting Gradlee configuration to your Android project. In your root `build.gradle` file:
```groovy
allprojects { 
	repositories 
		{ 
...
* Open app level build.grdle file and add .aar file and kotlin runtime
```groovy
dependencies 
{ 
...
* Add the follow permissions into manifest.xml
```xml
...
* Add the follow service declarations into manifest.xml
```xml
